...
fn cull_main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // MSAA: ensure at least 1px billboard padding for sub-sample coverage
  if cull.aa_mode == 4u {
    let msaa_pad = vec2<f32>(2.0 / cull.viewport_size.x, 2.0 / cull.viewport_size.y);
    ndc_min = ndc_min - msaa_pad;
    ndc_max = ndc_max + msaa_pad;
  }
